...
class Order(BaseModel):
    ...
    full_sell_amount: str = Field(..., alias="fullSellAmount", description="Full sell amount in wei")
    buy_amount: str = Field(..., alias="buyAmount", description="Amount to buy in wei")
    full_buy_amount: str = Field(..., alias="fullBuyAmount", description="Full buy amount in wei")
    ...
    partially_fillable: bool = Field(False, alias="partiallyFillable", description="Whether order can be partially filled")
    pre_interactions: List[dict] = Field(default_factory=list, alias="preInteractions", description="Pre-interactions")
    post_interactions: List[dict] = Field(default_factory=list, alias="postInteractions", description="Post-interactions")
    sell_token_source: str = Field("erc20", alias="sellTokenSource", description="Sell token source")
    buy_token_destination: str = Field("erc20", alias="buyTokenDestination", description="Buy token destination")
...
class Auction(BaseModel):
    ...
    tokens: Dict[str, Token] = Field(default_factory=dict, description="Token information")
    orders: List[Order] = Field(default_factory=list, description="Orders in the auction")
    liquidity: List[Liquidity] = Field(default_factory=list, description="Liquidity sources")
    effective_gas_price: str = Field(..., alias="effectiveGasPrice", description="Effective gas price in wei")
