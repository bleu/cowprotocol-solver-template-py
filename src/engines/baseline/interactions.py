...
class InteractionEncoder:
    ...
    def encode_generic_swap(
        ...
        """
        Encode a generic swap interaction.
        ...
        return Interaction(
            target=target,
            value=str(value),
            call_data=calldata.hex() if isinstance(calldata, bytes) else calldata
        )
    
    def build_interactions_for_path(
        self,
        path: List[str],
        pools: List[any],
        amounts: List[int]
        ...
        """
        Build interactions for a complete trading path.
        
        Args:
            path: List of token addresses in the path
            pools: List of pools for each hop
            amounts: List of amounts for each hop
        ...
        for i in range(len(path) - 1):
            ...
            pool = pools[i]
            amount_in = amounts[i]
            amount_out_min = amounts[i + 1] if i + 1 < len(amounts) else 0
            
            # Encode based on pool type
            # This is simplified - real implementation would check pool type
            interaction = self.encode_uniswap_v2_swap(
                pool_address=pool.address if hasattr(pool, 'address') else str(pool),
                token_in=token_in,
                token_out=token_out,
                amount_in=amount_in,
                amount_out_min=int(amount_out_min * 0.97)  # 3% slippage
            )
