...
class CowMatcher:
    ...
    def _build_order_book(self, orders: List[Order]) -> Dict[Tuple[str, str], List[Order]]:
        """
    ...
    def _can_match(self, order_a: Order, order_b: Order) -> bool:
        ...
        if not (order_a.sell_token.lower() == order_b.buy_token.lower() and
                order_a.buy_token.lower() == order_b.sell_token.lower()):
            return False
    ...
    def _create_match(self, order_a: Order, order_b: Order) -> Optional[OrderMatch]:
        ...
        traded_amount = min(
            int(sell_order.sell_amount),
            int(buy_order.buy_amount)
        )
    ...
    def _is_fillable(self, order: Order) -> bool:
        ...
        return (
            int(order.sell_amount) > 0 and
            int(order.buy_amount) > 0
        )
