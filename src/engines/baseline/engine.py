...
from src.engines.baseline.path_finder import PathFinder, TradePath
...
class BaselineEngine:
        ...
        try:
            ...
            if graph and pools:
                ...
                for order in remaining_orders:
                    ...
                    result = self._route_order_through_amms(
                        order, graph, pools
                    )
    ...
    def _extract_reference_prices(
        ...
        for token_addr, token_info in tokens.items():
            ...
            if hasattr(token_info, 'reference_price') and token_info.reference_price is not None:
                prices[token_addr] = int(token_info.reference_price)
            elif hasattr(token_info, 'external_price') and token_info.external_price is not None:
                ...
                if 'reference_price' in token_info and token_info['reference_price'] is not None:
                    prices[token_addr] = int(token_info['reference_price'])
                elif 'external_price' in token_info and token_info['external_price'] is not None:
                    price_decimal = float(token_info['external_price'])
                    prices[token_addr] = int(price_decimal * 10**18)
